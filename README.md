...
로컬 LLM은 기본적으로 Ollama의 `qwen3.5:4b-q4_K_M`을 사용합니다. 최초 설정에서는 약 3.4GB 모델을 내려받으므로 시간이 걸릴 수 있습니다.
화면 왼쪽 아래의 `설정`에서 Qwen 3.5 0.8B(약 1.0GB), 2B(약 1.9GB), 4B(약 3.4GB), 9B(약 6.6GB) 중 하나를 선택할 수 있습니다. 선택한 모델 하나를 전사 보정·요약·용어 탐지·퀴즈·질문 답변에 모두 사용합니다. 설치되지 않은 모델은 설정 저장 시 한 번 다운로드하며 이후에는 재사용합니다. 모델이 작을수록 연산량과 발열이 줄어드는 대신 긴 요약과 퀴즈의 품질이 낮아질 수 있으며, 0.8B와 2B는 저사양 기기 및 실험용 선택지입니다.
