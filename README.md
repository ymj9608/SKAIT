...
화면 왼쪽 아래의 `설정`에서 로컬 LLM 자원 사용량을 선택할 수 있습니다.

- `절전`: 2K 컨텍스트를 사용하고 매 응답 직후 모델을 메모리에서 내리며 생성 요청을 한 번에 하나씩 처리합니다. 발열과 대기 RAM을 가장 많이 줄이지만 다음 요청이 느리고 긴 자료의 응답 품질이 낮아질 수 있습니다.
- `균형`(기본값): 4K 컨텍스트를 사용하고 응답 후 2분 동안 모델을 유지하며 생성 요청을 한 번에 하나씩 처리합니다.
- `성능 우선`: `.env`의 `OLLAMA_CONTEXT_WINDOW`와 `OLLAMA_KEEP_ALIVE` 값을 그대로 사용합니다(기본 8K, 15분).

컨텍스트가 작을수록 추가 메모리 사용량은 줄지만, 모델을 실행하는 동안에는 모델 자체 메모리가 필요합니다. 설정을 변경하면 기존에 로드된 모델을 한 번 내린 뒤 다음 요청부터 새 값으로 다시 로드합니다. 시작 시 기본 모드는 `backend/.env`의 `OLLAMA_PERFORMANCE_MODE`로도 지정할 수 있습니다.
