...
- Zoom 브라우저 탭, YouTube 브라우저 탭 또는 마이크 음성을 30초 단위로 실시간 STT
...
YouTube URL은 영상 탭을 여는 링크로만 사용합니다. 백엔드는 URL에 접속하거나 `yt-dlp` 같은 도구로 미디어를 내려받지 않습니다. MLX Whisper가 파일 경로를 요구하므로 전송된 30초 조각은 운영체제 임시 폴더에 잠깐 생성되지만, 변환 성공·실패와 관계없이 즉시 삭제됩니다.
...
6. YouTube 탭에서 영상을 재생합니다. 약 30초마다 전사 구간이 추가됩니다.
