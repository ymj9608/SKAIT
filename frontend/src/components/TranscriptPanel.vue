...
const pendingSummaryCards = computed(() => props.summaryCards
  .filter((card) => summaryDrafts.value.cards[card.id])
...
const pendingSummaryNotes = computed(() => props.summaryNotes
  .filter((note) => summaryDrafts.value.notes[note.id] !== undefined)
...
const hasInvalidSummaryEdit = computed(() => {
  const cardsInvalid = props.summaryCards.some((card) => {
    const draft = summaryDrafts.value.cards[card.id]
  ...
  const notesInvalid = props.summaryNotes.some((note) => (
    !(summaryDrafts.value.notes[note.id] ?? '').trim()
  ))
...
async function submitSummaryEdits() {
  ...
  if (!payload.cards.length && !payload.notes.length) {
    ...
    <header class="panel-header">
      ...
      <div class="panel-actions">
        <button
          ...
          :disabled="!feedItems.length || recording || processing"
